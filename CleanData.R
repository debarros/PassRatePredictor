...
CleanData = function(d1, d2, regents){
  ...
  #Remove rows that have no student number
  d1 = d1[!is.na(d1$StudNum),]
  d2 = d2[!is.na(d2$StudNum),]
  regents = regents[!is.na(regents$StudNum),]
  ...
  for (i in 1:nrow(d1)){                                                     #For each score in the old predictor scores files,
    if (d1$StudNum[i] %in% regents$StudNum){                                 #if this student has an outcome score in the regents file,
      d1$Regents[i] = regents$Score[regents$StudNum == d1$StudNum[i]] #put the outcome score in that row, pairing it with the predictor score.
  ...
  d1 = d1[d1$drop == 0,]         #Subset d1 to only the rows with complete data
  ...
  d2 = d2[d2$drop == FALSE,]     #Subset d2 to only those rows that have a student number
  ...
  d1$Passed[d1$Regents >= 65] = 1  #For the students with regents scores of least 65, set "Passed" to 1 
